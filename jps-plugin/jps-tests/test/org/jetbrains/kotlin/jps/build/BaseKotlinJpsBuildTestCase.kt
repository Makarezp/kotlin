...
abstract class BaseKotlinJpsBuildTestCase : JpsBuildTestCase() {
    ...
    private fun setKeepAlive(newValue: String?) {
        when (newValue) {
            null -> System.setProperty(KOTLIN_COMPILER_ENVIRONMENT_KEEPALIVE_PROPERTY, newValue)
            else -> System.clearProperty(KOTLIN_COMPILER_ENVIRONMENT_KEEPALIVE_PROPERTY)
        }
    }
}